...
d1 <- data.frame(MRSTATEPSTL = "AK", DOB_MM = NA, ptb_prop = NA, ptb_prop2 = NA, year = NA, 
                 ...
                 Section_1115_waiver = 0, Expansion_type= "Full", Notes_births = NA, Notes = NA, FIPS = 2, n=NA,
...
#  print(iteration)
  ...
  ##############################################################################################################################
  dat <- dat %>% mutate(ptb_count = ptb_prop*n) 

  dat <- dat %>% group_by(State) %>%  mutate(Y = rpois(n=num_months, lambda = exp(log(ptb_count) + log(CTE)*A)))
...
#  dat <- dat %>%  mutate(Y = intercept + A*CTE + e_Y)
  ...
  # estimate effects using TMLE 
  m3 <- tmle(Y=dat$Y, A=dat$A, W=dat[,"State"])
  ...
  df_cte <- data.frame(rbind(cbind(estimator = "truth", result = cte_truth, lb = NA, ub = NA), 
                            ...
                            cbind(estimator = "TMLE", result=m3$estimates$ATT$psi, 
                                  lb = m3$estimates$ATT$CI[1], 
                                  ub = m3$estimates$ATT$CI[2])))
  ...
  # estimate effects using TMLE 
  m3_hte <- tmle(Y=dat$Y, A=dat$A, W=dat[,"State"])
  ...
  df_hte <- data.frame(rbind(cbind(estimator="truth", result=hte_truth, lb = NA, ub = NA), 
                             ...
                             cbind(estimator = "TMLE", result=m3_hte$estimates$ATT$psi, 
                                   lb = m3_hte$estimates$ATT$CI[1], 
                                   ub = m3_hte$estimates$ATT$CI[2])))
...
results_ls <- lapply(1:20, function(x) sim_rep(x, dat=dat, CTE = -0.02, HTE = c(-0.01, -0.02), DTE = c(-0.01, -0.015, -0.02)))
