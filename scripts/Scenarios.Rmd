...
ggplot(s1, aes(x = time, y = outcome)) + 
  geom_point(aes(fill = state), size = 4, pch = 21) +
  geom_text(aes(label = outcome), nudge_x = 0.075) +
  scale_fill_manual(values = c("#a6cee3","#005a32")) + 
  theme_bw()
```
(WHAT DO YOU THINK ABOUT ADDING LINES CONNECTING THE OBSERVED PRE-POST VALUES FOR EACH STATE? 
I ALSO THINK WE SHOULD MAKE THE TREATED STATE DISTINCT IN SOME WAY)
...
1) parallel trends, or counterfactual parallel trends as described by Ben-Michael (CAN WE SAY WHAT THIS IS? I'M NOT SURE WHAT YOU MEAN OR HOW IT'S DIFFERENT),
2) No anticipation, 
3) Policy is the only thing that changes at the time of treatment that affects the outcome

WONDERING IF IT MAKES SENSE TO ALSO PHRASE THESE ASSUMPTIONS IN A EPI-CAUSAL INFERENCE WAY... LIKE NO INTERFERENCE, CONSISTENCY, POSITIVITY, EXCHANGEABILITY, ETC -- IF WE DO THIS, DO WE NEED TO ASSUME LINEARITY? 
...
s2a_plot <-ggplot(s2a, aes(x = time, y = outcome)) + 
  ...
  scale_fill_manual(values = c("#a6cee3","#005a32", "#99000d")) +
...
```
(AS ABOVE, POSSIBLE TO ADD LINES CONNECTING STATES IN PRE-POST TIMES, AND TO INDICATE WHICH IS TREATED?)
...
the effect separately by time since treatment. To do that, we could label the 
outcomes separately by time since treatment and calculate a separate DID for 
every time point since treatment. The average of these DIDs will equal the 6
that we calculated using the average outcome over post-treatment time.

WHY WOULD WE WANT TO CALCULATE THE EFFECTS SEPARATELY BY TIME SINCE TREATMENT? I THINK 
IT'S WORTH SPELLING OUT WHY THAT WOULD BE USEFUL, AND SHOWING AN EXAMPLE OF HOW TO DO IT
-- OR IS THIS WHAT YOU'RE DOING BELOW? I WOULD CLARIFY
