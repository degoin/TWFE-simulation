...
DOES control_group = "notyettreated" CHANGE WHICH CONTRASTS GO INTO THE SIMPLE 
EFFECT ESTIMATE? IT SEEMS LIKE IT SHOULD BUT I SHOULD DOUBLE CHECK WITH THE DATA
AND DISCUSS WITH DANA.

SHOULD I ADD CONTROL GROUP = "NOT YET TREATED" FOR LATER SCENARIOS TO BE CONSISTENT
  OR TO COMPARE THE EFFECTS OF THIS PARAM ON THE ESTIMATION?
...
they may change over time and that these changes may affect the readers ability 
...
8) **Correct model specification**: SOMETHING ABOUT LINEARITY? ALSO RELATED TO THE PARALLEL TRENDS ASSUMPTION 
...
## Scenario 3A and 3B (Two states, dynamic treatment effect)
...
Scenario 5 introduces staggered timing. In this example, the treated states introduce the policy at two different time points.
...
before, we display the output from this step using `summary()`. This output shows the estimated ATT for each combination of treated state and time. The time is long
becuase it includes estimates for pre-policy time, which helps with the 
...
while at other points, two comparisons contribute. But this "knowledge" is lot
...
So, it isn't straightforward what the estimate should be when summarizing across 
several contrasts. But what we show below based on the Goodman Bacon decomposition
is that the TWFE estimator is influenced by the forbidden fourth contrast that
doesn't meet the parallel trends assumption, so we'd like an estimator that doesn't
suffer form this concern.
...
Notice that our fourth contrast (here shown in row 3 of the table) contributes
15% of the weight to the estimate. We want an estimator that does not use this
contrast! 
...
Let's aggregate the ATTs using `type = simple`. Callaway and Sant'Anna's estimate considers only the clean contrasts of 1 and 2 and combines them by weighting by the total
time in the post-treatment period. For contrast 1, there are 16 
time periods post treatment and for contrast 2, there are 9 periods 
...
regression and `filter()` out the policy estimates since the regression table 
is so large:
...
This scenario increases the number of treated and control units, as is common in 
analyses across several geographic areas like states. We also added variability
to the outcome, with different amounts of variability by state to reflect units
containing varying sample sizes.

```{r, echo=F}

set.seed(112358)

#expand on the s7 dataset to include many states 
state_base1 <- s7 %>% filter(state == 1)
...
s8_comp4 <- ggplot(all_states %>% filter(state >= 30, time >= 5), aes(x = time, y = outcome)) +
  geom_line(aes(col = factor(time_first_treat), group = state)) + 
...
s8_comp3 <- ggplot(all_states %>% filter(state >= 30, time < 12), aes(x = time, y = outcome)) +   
  geom_line(aes(col = factor(time_first_treat), group = state)) + 
  geom_point(aes(fill = factor(time_first_treat), group = state, pch = ever_trt), show.legend = F) +
...
Up until now, we haven't worried about the estimation of the 95% confidence interval. 
In particular, we need to account for the clustering on data over time within state.
To do that we use the `vcovHC` function from the `sandwich` package. 
...
We can use the Bacon decomposition to examine the contrasts contributing to the 
TWFE estimate. I set up this scenario such that the effects should be the same
as for scenario 7. In that scenario the estimates equalled:

i) Treated at time 5 vs. never-treated: 18 
ii) Treated at time 12 vs. never-treated: 5
iii) Treated at time 12 vs post-treatment time for states treated at time 5: -11
iv) Treated at time 5 vs pre-treatment time for states treated at time 12: 9
...
We can see that the estimates are not too far off. Comparing to the decomposition
table from scenario 7, the weights have shifted. In particular here 24% of the weight
is put on the later treated vs. never treated states, whereas this number was 47%
in scenario 7. Worryingly, the weight on the forbidden contrast increased from 15%
in scenario 7 to 31% in this scenario. This is because the weight is proportional to **....[TO FILL IN]**
...
Their weighted estimator is equal to $18 \times (25*6) + 5 \times (25*6)$
...
summary(s8_cs_ag2)
```
(Was expecting 18 and 5 -- possible we are off just due to random error - need to 
...
assumption by investigating if the pre-treatment CIs overlap the null and are
