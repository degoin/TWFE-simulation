...
1. Goin DE, Riddell CA (2022). New developments in difference-in-differences designs. Pre-print. ADD LINK.

**Bug reports**

Submit any bug reports to: c.riddell [AT] berkeley [DOT] edu or open an issue on [Github](https://github.com/degoin/TWFE-simulation).
...
Notice that our fourth contrast (here shown in row 3 of the second table) contributes
...
summary(s7_cs_ag)
```
**[DANA: CAN YOU CONFIRM YOU AGREE WITH WHAT I WROTE ABOVE? specifically how the 
simple estimate is calculated.]**
...
NOTE: (Was expecting 18 and 5 as the group specific estimates -- possible we are
off just due to random error - need to rerun with a different seed or with less
variability and see if that is true)
...
time before the first intervention point to 5 time units, rather than the 15 
in Scenario 8. If you look back at the illustration of the previous scenario,
this change should not effect the estimates of the effects since...[FINSISH]. 

The goal is to see how the different estimators are affected by 
this change. 
...
Again, we need to use the `vcovHC` function to estimate the SE and obtain the 
...
to `r round(ub2, 1)`. Let's compare thish with the estimate from the previous Scenario:
...
As you can see the estimate for Scenarion 9 is much lower than that for Scenario
8. Let's see if the weights from the Bacon decomposition can hint at why.
...
And again recall the weights from Scenario 8:
...
We can see that more weight is put on the states treated at time=22 in Scenario
9 vs. 8 and less weight on the states treated at time=15. This is because the 
TWFE estimator is a weighted combination, where the weights are partially 
determined by how close the treatment change is to the middle of the panel, with
more weight put on the estimates for groups with changes closer to the middle of
...
This weighting is done by the TWFE but does not likely correspond to any weighting 
that you, the researcher, would choose. Concerning!

Let's see how the other estimators are effected by this change:
...
more weight on the constrasts which are closer to the middle of the panel. The 
